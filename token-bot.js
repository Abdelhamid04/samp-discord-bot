...
app.get('/validate-ip/:ip', (req, res) => {
    ...
    if (!ip) return res.status(400).json({ valid: false, error: 'Missing IP' });

    db.get('SELECT * FROM launcher_tokens WHERE player_ip = ? AND used = 0 ORDER BY created_at DESC LIMIT 1', [ip], (err, row) => {
        if (err) return res.status(500).json({ valid: false, error: 'Database error' });
        if (!row) return res.status(401).json({ valid: false, error: 'Token not found for IP' });
        ...
        db.run('UPDATE launcher_tokens SET used = 1, player_name = ? WHERE token = ?', [playerName || row.player_name, row.token], (updateErr) => {
            if (updateErr) return res.status(500).json({ valid: false, error: 'Database update error' });
            res.json({ valid: true, token: row.token, machine_name: row.machine_name, user_name: row.user_name, age_seconds: age });
        });
    });
});
...
app.get('/validate/:token', (req, res) => {
    ...
    if (!/^[A-Z0-9]{32}$/.test(token)) return res.status(400).json({ valid: false, error: 'Invalid token format' });

    db.get('SELECT * FROM launcher_tokens WHERE token = ? AND used = 0', [token], (err, row) => {
        if (err) return res.status(500).json({ valid: false, error: 'Database error' });
        if (!row) return res.status(401).json({ valid: false, error: 'Token not found or already used' });
        ...
        res.json({ valid: true, machine_name: row.machine_name, user_name: row.user_name, age_seconds: age });
    });
});
